...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> startServer(String ip) async {
    ...
    while (!started && tryPort <= 65535) {
      try {
        ...
        if (tryPort > 65535 && mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('无法找到可用端口，启动失败'),duration: Duration(seconds: 1)),
          );
        }
      }
    }
    ...
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('服务器已启动: ${serverUrl ?? "所有地址"}'),duration: Duration(seconds: 1)),
    );
  }
  ...
  Future<void> stopServer() async {
    if (server != null) {
      try {
        ...
        if (!mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('服务器已停止'),duration: Duration(seconds: 1)),
        );
      }
    }
  }
                  ...
                  if (!running) ...[
                    ...
                    if (ipList.isEmpty)
                      SizedBox(
                        ...
                        child: Center(
                          child: Column(
                            ...
                            children: const [
                              Icon(Icons.arrow_downward, size: 40, color: Colors.grey),
                              SizedBox(height: 8),
                              Text(
                                '未检测到可用 IP\n下拉刷新试试',
                      ...
                      ...ipList.map((ip) {
                        ...
                        return ListTile(
                          ...
                          title: Text(displayText, style: const TextStyle(color: Colors.blue)),
                    ...
                    Padding(
                      ...
                      child: ElevatedButton(
                        onPressed: canStart ? () => startServer(selectedIp!) : null,
